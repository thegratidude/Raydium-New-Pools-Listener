...
def buy(pair_address: str, sol_in: float = 0.1, slippage: int = 1) -> bool:
    try:
        ...
        # Cache the token account address for fast sell
        with open(TOKEN_ACCOUNT_CACHE_FILE, 'w') as f:
            f.write(str(token_account))
        print(f"Cached token account address to {TOKEN_ACCOUNT_CACHE_FILE}: {token_account}")
...
def sell(pair_address: str, percentage: int = 100, slippage: int = 1):
    try:
        ...
        # Use cached token account address
        try:
            with open(TOKEN_ACCOUNT_CACHE_FILE) as f:
                token_account = Pubkey.from_string(f.read().strip())
            print(f"Using cached token account address: {token_account}")
        except Exception as e:
            print(f"Error reading cached token account: {e}")
            return None, None
        ...
        print(f"Amount In: {amount_in} | Minimum Amount Out: {minimum_amount_out}")
        # token_account is now from cache
        ...
        print("Transaction confirmed:", confirmed)
        if confirmed:
            print(f"Sell complete. Tx hash: {txn_sig}, Amount out: {amount_out}")
            return txn_sig, amount_out
        else:
            return None, None
    except Exception as e:
        print("Error occurred during transaction:", e)
